...
- If Final_Instrument_Published_Name is blank but a normalized title exists → promote it ONLY if: (a) Final_Language_Doc is French or Spanish (keep normalized original-language title as-is per spec), OR (b) Final_Language_Doc is English, OR (c) Final_Language_Doc is unknown/blank (safe fallback). Do NOT copy the original-language normalized title into Final_Instrument_Published_Name for non-French/Spanish documents (e.g., Portuguese, Arabic, Thai). If no official English title is found and you cannot confidently translate, leave Final_Instrument_Published_Name blank and explain in Missing_Conflict_Reason.
