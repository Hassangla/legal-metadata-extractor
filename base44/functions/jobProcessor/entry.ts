...
const MODEL_PRICING = {
    ...
    'gpt-4.1-nano':{input:0.10,output:0.40},'gpt-4.5-preview':{input:75.00,output:150.00},'gpt-5':{input:10.00,output:40.00},'gpt-5-mini':{input:1.50,output:6.00},'gpt-5.4':{input:10.00,output:40.00},'gpt-3.5-turbo':{input:0.50,output:1.50},
...
The object has ONE top-level key "evidence" containing all evidence fields AND all Final_* decision fields:
{
  "evidence": {
    ...
    "Final_Flag": ""
                        ...
                        console.log(`[DIAG] row=${row.row_index} provider=${providerType} contentLen=${(content||'').length}`);
