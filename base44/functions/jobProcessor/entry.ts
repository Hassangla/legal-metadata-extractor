...
function resolveEconomyCode(raw,map){const k=(raw||'').toLowerCase().trim();if(!k)return{code:'',ecoAlias:null,ecoAliasTarget:null};if(map[k])return{code:map[k],ecoAlias:null,ecoAliasTarget:null};const t=ECONOMY_ALIASES[k];if(t){return{code:map[t.toLowerCase().trim()]||'',ecoAlias:k,ecoAliasTarget:t};}return{code:'',ecoAlias:null,ecoAliasTarget:null};}
